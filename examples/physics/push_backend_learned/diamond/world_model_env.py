"""World Model Environment for diffusion-based inference."""
...
from .rew_end_model import RewEndModel
...
class WorldModelEnv:
	...
	@torch.no_grad()
	def predict_next_obs_batch(self, action_batch: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
		"""
		Parallel prediction of next observations for multiple actions.
		GPU parallelizes diffusion sampling for all actions.

		Args:
			action_batch: [num_actions, action_dim] or [num_actions]

		Returns:
			next_obs_batch, rew_batch, end_batch, trunc_batch
		"""
		num_actions = action_batch.size(0)
		device = self.device

		obs_batch = self.obs_buffer.expand(num_actions, -1, -1, -1, -1).contiguous()
		act_batch = self.act_buffer.expand(num_actions, -1, -1).contiguous()
		act_batch[:, -1] = action_batch

		next_obs_batch, _ = self.sampler_next_obs.sample(
			obs_batch[:, self.n_skip_next_obs:],
			act_batch[:, self.n_skip_next_obs:]
		)

		if self.sampler_upsampling is not None:
			obs_full_batch = self.obs_full_res_buffer.expand(num_actions, -1, -1, -1, -1).contiguous()
			low_res = F.interpolate(
				next_obs_batch,
				scale_factor=self.sampler_upsampling.denoiser.cfg.upsampling_factor,
				mode="bicubic"
			).unsqueeze(1)
			next_obs_full_batch, _ = self.sampler_upsampling.sample(
				torch.cat((obs_full_batch[:, self.n_skip_upsampling:], low_res), dim=1),
				None
			)
			next_obs_batch = next_obs_full_batch

		if self.rew_end_model is not None:
			obs_last = self.obs_buffer[:, -1:].expand(num_actions, -1, -1, -1, -1).contiguous()
			act_last = self.act_buffer[:, -1:].expand(num_actions, -1, -1).contiguous()
			hx_batch = self.hx_rew_end.expand(-1, num_actions, -1).contiguous()
			cx_batch = self.cx_rew_end.expand(-1, num_actions, -1).contiguous()
			...
			rew_batch = Categorical(logits=logits_rew).sample().squeeze(1) - 1.0
			end_batch = Categorical(logits=logits_end).sample().squeeze(1)
		else:
			rew_batch = torch.zeros(num_actions, dtype=torch.float32, device=device)
			end_batch = torch.zeros(num_actions, dtype=torch.int64, device=device)

		trunc_batch = torch.zeros(num_actions, dtype=torch.int64, device=device)

		return next_obs_batch, rew_batch, end_batch, trunc_batch
