"""Isolate and validate the clear primitive on occluded_block."""
...
def main():
    ...
    obstacle = env.unwrapped.named_objects["obstacle_front_block"]
    initial = obstacle.pose.p.detach().clone()
    executor = PrimitiveExecutor(backend, env, kp_pos=args.kp_pos, kp_ori=args.kp_ori)
    result = executor.execute(
        PrimitiveAction(
            "clear",
            {
                "obstacle_id": "front_block",
                "direction": [1.0, 0.0],
    ...
    final = obstacle.pose.p.detach().clone()
